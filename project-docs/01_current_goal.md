# Current Goal

## Active Goal

Complete the first migration foundation for Vercel + Supabase.

## Immediate Objectives

1. Replace frontend auth assumptions from local session auth to Supabase Auth
2. Introduce app-level profile schema groundwork
3. Create a durable `project-docs` and handoff workflow
4. Prepare the codebase for later removal of legacy Express auth
5. Make documentation updates a mandatory end-of-task rule for any agent touching the repo

## In Scope Right Now

- Frontend auth layer
- Supabase client bootstrap
- Profile schema preparation
- Migration docs and operating docs

## Not In Scope Right Now

- Full API server rewrite
- Full RLS implementation
- Production deployment setup completion
- Dashboard logic rewrite
- Next.js migration
